...
function App() {
  ...
  return (
    <>
      <div className="border border-white rounded-lg min-w-[100%] max-w-[590px] h-fit m-auto px-8 py-4 bg-white shadow-2xl shadow-[hsl(292,16%,49%)]">
        <div>
          ...
          {faqData.map((item, index) => {
            return (
              <div key={index}>
                <button
                  className={`w-[100%] flex items-center justify-between pt-3 pb-3 cursor-pointer  font-semibold text-wrap ${
                    activeIndex === index
                ...
                >
                  <p className="pb-4 bg-white">{item.answer}Hello</p>
                </div>
              </div>
            );
          })}
